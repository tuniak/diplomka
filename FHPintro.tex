...
If the cell is occupied by a particle, that means $n_i(\bf{r})=1$, it propagates along the corresponding lattice vector to the neighboring node, see the figure \ref{FHPprop}.
...
P(0,s(.)) \geq 0.
\end{align*}
As always, probabilities over whole lattice are normalized so
\begin{align*}
\sum_s(.) P(0,n(.)) = 1.
\end{align*}
...
\mathcal{E} n(t,.) = n(t+1,.)
\end{equation*}
However, for FHP and other non-deterministic models, the conservation of probability is governed by the more general formula
\begin{equation}
